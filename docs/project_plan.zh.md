...
**AgentTest** —— 面向 Java AI agent 代码的 **agent-aware** 单元测试生成器。

生成的测试覆盖三类不变量,这些都是通用 Java 测试生成器(TestSpark、
ChatUniTest、Diffblue 等)漏掉的 —— 因为它们没有 agent 分类法、也不懂
Spring AI / LangChain4j / MCP:

1. **安全性**(OWASP-anchored)—— prompt 注入、敏感数据泄漏、multi-tenant
   边界违规、tool 描述与实现不一致带来的 excessive agency。
2. **Agent 模式正确性** —— tool 描述与实现的契约一致性、prompt 模板在
   refactor 下的稳定性、RAG context 处理。
3. **可靠性** —— retry / circuit-breaker 边界、瞬态故障下的幂等性。

OWASP 是**评测的 ground truth**:eval harness 只对能用确定性注入合成的
风险打分(§ 5),也就是上述安全性这一类。另外两类走同一条 pipeline
(analyzer → retrieval → generator → validator),也会出现在测试集里,
但**不计入主指标的 recall / precision** —— 主指标必须 model-as-judge-free
是作业的硬约束。生成器的覆盖面比 eval 的打分面更宽,这个不对称是有意的。
...
是 *agent 特定* 的测试 —— 通用 Java 测试生成器看不懂的不变量:

- **安全性**(OWASP-anchored):prompt 模板组装时的注入漏洞、敏感数据漏到
  prompt 或日志、multi-tenant 边界违规、tool 描述与实现的偏离(excessive
  agency)。
- **Agent 模式正确性**:tool schema 与实现的契约一致性、prompt 模板跨 refactor
  的稳定性、RAG context 不变量(只有检索到的 context 进 prompt)。
- **可靠性**:retry / circuit-breaker 误配、瞬态故障下的幂等性。

其中**安全性**这一项恰好就是 OWASP Top 10 for LLM Applications、OWASP LLMSVS、
OWASP Top 10 for Agentic AI 列的风险 —— 也是我们做**客观评测**用的那一类
(§ 5),因为只有 OWASP 风险能用确定性注入合成 ground truth。另外两类走的是
同一条 pipeline。**通用**的 AI 测试生成工具(TestSpark、MutGen、ChatUniTest、
Diffblue)只优化通用代码上的 line coverage 和 mutation score,**没有 agent
分类法、没有 Spring AI / LangChain4j / MCP 知识**,所以系统性地漏掉这一类 bug。
...
特殊 bug 上 —— prompt injection、tool 契约漂移、敏感数据外泄、multi-tenant
边界失败、瞬态故障下的 retry / 幂等性破坏。这些 bug 又恰好对**传统测试套件
不可见**(传统测试测功能正确性,不测对抗鲁棒性、也不测 agent 模式契约)。
把"通用 AI test gen 工具会写的"和"agent 代码实际需要测的"之间的差距填上,
每周给开发者省下几小时手写这一类测试的时间,降低 agent 类 bug —— 安全、
正确性、可靠性都算 —— 不知不觉上线的概率。
...
5. **Aggregator。** 把活下来的 method 合并进一个稳定命名的类
   (`<TargetClass>AgentGenTest.java`),解决 import,头部注释列出覆盖
   的 OWASP 风险 ID
