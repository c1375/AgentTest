...
pattern,每条 ~150 token)。两者都是 YAML + embedded 索引
(`text-embedding-3-small`)。每个风险位点,从 pattern 库检索 top-3 +
匹配的 OWASP 条目,塞进 generator prompt。**RAG(对照"把整目录硬塞进
每个 prompt")是否真的有用,通过 ablation 实证检验** —— 见 § 5。
...
- *隐私*:AgentTest **不收集、不写日志、不持久化**任何用户数据。但
  提交的 Java 源**会传给 Anthropic** 做 Sonnet 调用;**启用 pattern
  库 RAG 时,位点上下文片段还会传给 OpenAI** 做 `text-embedding-3-small`
  嵌入。README 明示这两个 provider,让用专有代码的用户在跑工具之前
  就能自己判断
- *API key*:README 列出 grader 需要在 `.env` 配的 key —— `ANTHROPIC_API_KEY`
  (生成必需)、`OPENAI_API_KEY`(仅 RAG 启用时需要,smoke test 默认
  关闭)
