...
public class OrchestratorWorkers {
	 ...
	 * The orchestrator is an LLM whose output is untrusted; without a cap a
	 * single (possibly poisoned) orchestrator response could spawn an
	 * arbitrary number of worker LLM calls (cost amplification, runaway
	 * cascade — OWASP LLM06 / ASI08).
	 */
	...
	public FinalResponse process(String taskDescription) {
		...
		// Bounded fan-out: refuse to dispatch more than MAX_WORKERS subtasks.
		// The orchestrator's task list is LLM-controlled; without this guard a
		// single response could trigger arbitrarily many worker LLM calls.
		if (orchestratorResponse.tasks() != null
				&& orchestratorResponse.tasks().size() > MAX_WORKERS) {
			throw new IllegalStateException(
					"Orchestrator returned " + orchestratorResponse.tasks().size()
							+ " tasks; max permitted is " + MAX_WORKERS);
		}
